...
namespace CSharpBoiler.NetworkHelper
{
    static class DataSender
    {
        ...
        public static void Send(CMsgGCCStrike15_v2_MatchList mainMatchList)
        {
            if (socketClient == null)
                return;

            // Connect to the remote endpoint.
            socketClient.BeginConnect(ipAddress, port,
                new AsyncCallback(ConnectCallback), socketClient);
            connectDone.WaitOne();

            //Send data to the remote device.
            //Serialize Matchlist to string
            MemoryStream memoryStream = new MemoryStream();
            Serializer.Serialize<CMsgGCCStrike15_v2_MatchList>(memoryStream, mainMatchList);
